---
title: "Change in Covid Attitudes in New Zealand from 2021 - 2022"
execute:
  warning: false
  echo: false
draft: false
...
We investigate change in attitudes to COVID-19 from the Time 12 and Time 13 of the New Zealand Attitudes and Values Study (NZAVS). The NZAVS is a national probability panel study of attitudes and values in New Zealanders, started by Prof. Chris G. Sibley in 2009. 
See: [Link on this site](https://go-bayes.github.io/b-causal/posts/nzavs/)

[NZAVS homepage](https://www.psych.auckland.ac.nz/en/about/new-zealand-attitudes-and-values-study.html)

The sample consists of all those participants who responded in Time 12 (N = 38,551). The sample in Time 13 was (n = 28,642). Such attrition, or loss to follow-up, results in bias.  
...
##  Boxplot reveals considerable change in COVID attitudes
...
gg2 <- plot(ggpredict(m2, terms = "Wave"),
            ...
            dot.alpha = .005) +
  scale_y_continuous(limits = c(1, 7)) +
  labs(title =  "CPVID Created Lab")

gg3 <- plot(ggpredict(m3, terms = "Wave"),
            add.data = TRUE,
            dot.alpha = .005) +
  scale_y_continuous(limits = c(1, 7)) +
  labs(title =  "Trust Govt Response")

gg4 <- plot(ggpredict(m4, terms = "Wave"),
            add.data = TRUE,
            dot.alpha = .005) +
  scale_y_continuous(limits = c(0, 10)) +
  labs(title =  "Satisfied Govt Response")
...
Overall, the New Zealanders do not perceive the risks of COVID-19 to be exaggerated, however there is increasing perception of exaggeration.  

There is increasing belief that COVID-19 was manufactured in a lab.

Overall, New Zealanders have high levels of trust in the Government response to COVID-19, but these levels are somewhat declining. 

Similarly,  New Zealanders are satisfied with the Government response to COVID-19, but levels of trust are somewhat declining. 

Importantly, these models do not adjust from bias from sample attrition. To the extent that institutional mistrust has declined during the past two years, we expect that changes in trust and satisfaction in the Government response to COVID-19 will have been somewhat steeper than presented here. 

In future work, we shall account for bias from sample attrition using multiple imputation. We will report our results here. Please stay tuned.
